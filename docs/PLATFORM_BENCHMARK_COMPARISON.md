...
AudioDecode delivers **dramatically better performance across all platforms**, making it ideal for both development and production deployments.

| Platform | OpenAI Whisper | AudioDecode | Speedup |
|----------|----------------|-------------|---------|
| **Mac (M-series) CPU** | 14.17s | 8.00s | **1.77x faster** ⚡ |
| **Linux Docker CPU** | 47.64s | 7.94s | **6.00x faster** ⚡⚡⚡ |
| **A10G GPU** | 22.58s | 9.26s | **2.44x faster** ⚡⚡ |
...
| System | Load Time | Memory |
|--------|-----------|--------|
| OpenAI Whisper | 1.09s | ~140 MB |
| AudioDecode | 0.33s | ~10 MB |
| **Improvement** | **3.3x faster** | **93% less** |

### Transcription Performance

| System | Time | RTF | Memory | vs OpenAI |
|--------|------|-----|--------|-----------|
| OpenAI Whisper (GPU) | 22.58s | 17.7x | ~100 MB | baseline |
| AudioDecode (GPU) | 9.26s | 43.8x | ~80 MB | **2.44x faster** |
| AudioDecode (optimized) | 7.70s | 52.6x | ~80 MB | **2.93x faster** |

### Total Pipeline

| System | Total Time | Speedup |
|--------|------------|---------|
| OpenAI Whisper | 23.67s | baseline |
| AudioDecode | **9.59s** | **2.47x faster** ⚡⚡ |

### Quality Metrics

| System | Words | Segments | Word Timestamps |
|--------|-------|----------|-----------------|
| OpenAI Whisper | ~880 | 54 | 0 |
| AudioDecode | ~882 | 14 | **890** ✨ |

**Quality**: 99.7% text similarity
...
- batch_size=24: 3.83s (105.8x RTF) - default
- batch_size=32: 3.82s (106.2x RTF)

**Key Insights:**
- batch_size=16 is optimal for A10G (not default 24)
- float16 compute type is best for GPU
- int8 slower on GPU (5.02s vs 3.74s)
