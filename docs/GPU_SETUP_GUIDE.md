...
- batch_size=16 is optimal (not default 24)
- Can achieve up to 108.3x RTF in isolated tests
- float16 compute type best for GPU
- Model loading: 0.2-0.3s (3x faster than OpenAI)

---

## Expected Results for Other GPUs
...
| **A10G GPU** | 22.58s | 9.26s | **2.4x** ✅ |
| **T4 GPU** | ~5s (est) | ~2s (est) | **~2-3x** |
| **RTX 4090** | ~3s (est) | ~1s (est) | **~3-4x** |
| **A100** | ~2s (est) | ~0.7s (est) | **~3-4x** |

**Note**: A10G validated. Other GPUs estimated based on A10G results and GPU specs.
